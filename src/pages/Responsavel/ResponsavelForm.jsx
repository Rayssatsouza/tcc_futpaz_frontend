...
export default function ResponsavelForm() {
    ...
    const getAtletaResponsavel = useCallback(
        () => {
            setLoad(true)
    ...
    const criarResponsavel = (body) => {
        ...
        post(`api/responsavel`, body).then((response) => {
            setSnackOptions(_ => ({ mensage: "Responsável criado com Sucesso", type: "success", open: true }));
            setLoad(false)
    ...
    }, [getAtletaResponsavel, getResponsavel, id, responsavel], [atletaResponsavelList]);
    ...
    const salvarAtleta = (bodyAtleta) => {
        post(`api/atletaResponsavel`, bodyAtleta).then((_) => {
            handleClose();
            getAtletaResponsavel();
        }).catch((erro) => {
            setSnackOptions(_ => ({
